...
**Revised 2026-07-13 (b):** During code review of Task 3.2 (compose file), found that `POSTGRES_PASSWORD` is interpolated unescaped into `DATABASE_URL: postgresql://openbrain:${POSTGRES_PASSWORD}@openbrain-db:5432/openbrain` — a password generated with a non-URL-safe tool (e.g. one that can emit `@`, `:`, `/`, `#`, `%`, `?`) would silently produce a malformed/misparsed connection string, surfacing later as a confusing `psycopg` connection failure during Task 3.3/4.1. Fixed by adding a one-line warning to `deploy/.env.example` recommending a URL-safe generator (`openssl rand -hex 32`). See updated Task 3.2 below.
